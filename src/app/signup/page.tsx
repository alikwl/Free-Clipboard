  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data?.user && data.user.identities && data.user.identities.length === 0) {
        setSuccessMsg('This email is already registered. Check your inbox or sign in.');
      ...
      <div className="safe-card relative z-10 w-full max-w-md">
        <div className="flex flex-col items-center gap-2 mb-8 text-center">
          ...
          <p className="text-sm text-neutral-500 font-medium">
            Create your account &mdash; free 7-day Pro trial included
          </p>
        </div>
        ...
        <Card className="border border-white/5 bg-black/40 backdrop-blur-md shadow-2xl">
          <CardHeader className="space-y-1 border-b border-white/5 pb-5">
            <CardTitle className="text-lg font-bold text-neutral-200">Get started</CardTitle>
            <CardDescription className="text-neutral-500 text-xs">
              No credit card required. Pro trial for 7 days.
            </CardDescription>
          </CardHeader>
          ...
          <CardFooter className="flex flex-wrap justify-center gap-1 border-t border-white/5 pb-4 pt-4 text-center text-xs text-neutral-500">
            ...
            <Link href="/login" className="flex items-center gap-0.5 font-bold text-indigo-400 hover:underline">
              Sign in <Sparkles className="w-3 h-3 text-indigo-400/80" />
            </Link>
          </CardFooter>
        </Card>
